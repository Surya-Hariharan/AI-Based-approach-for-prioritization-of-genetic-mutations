...
def main():
    ...
    # Check if X_all needs to be stacked if it's a list of arrays? No, extend works for list, need to verify shape.
    # X.cpu().numpy() returns (B, F). Extend adds rows. So X_all is list of rows? No, extend adds elements to list.
    # If X is (B, F), X.cpu().numpy() is array. Extend appends elements?
    # Wait, if X is 2D array, extend iterates over rows. So X_all becomes list of 1D arrays (rows).
    # Then np.array(X_all) becomes (N, F). Correct.
    ...
    enrichment = bio_eval.enrichment_analysis(ranked_df, top_k=min(50, len(ranked_df)))
